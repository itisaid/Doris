            ...
            public PeerCallback execute() throws AccessException {
                try {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("doPeerExecute operation:" + getName() + ",Key=" + operationData.getArgs().get(0)
                                     + ",Value len " + commuValue.getValueBytes().length + ",compressed?"
                                     + commuValue.isCompressed() + ",Datasource:" + dataSource);
                    }
